...
class EdgeDetector():
    ...
    def auto_mask_generate(self):
        generator = SamAutomaticMaskGenerator(self.sam)
        masks = generator.generate(self.image)

        max_area_mask = self.find_mask_with_second_max_bbox(masks)
        max_area_mask = [max_area_mask]

        px = 1/plt.rcParams['figure.dpi']  # pixel in inches
        fig, ax = plt.subplots(figsize=(288*px, 216*px))
        ax.imshow(self.image)
        self.show_anns(max_area_mask, ax)
        # self.show_mask(max_area_mask[0], ax)
        ax.axis('off')
    ...
    def find_mask_with_second_max_bbox(self, masks):
        bboxes = [mask['bbox'] for mask in masks]
        areas = [(bbox[2] - bbox[0]) * (bbox[3] - bbox[1]) for bbox in bboxes]
        sorted_indices = np.argsort(areas)
        second_max_index = sorted_indices[-2] if len(sorted_indices) > 1 else sorted_indices[0]
        return masks[second_max_index]


        # bbox = [mask['area'] for mask in masks]
        # sorted_areas = sorted(bbox, reverse=True)
        # if len(sorted_areas) < 2:
        #     return None
        # second_max_area = sorted_areas[1]
        # for mask in masks:
        #     if mask['area'] == second_max_area:
        #         return mask
        # max_area = 0
        # best_mask = None
        # for mask in masks:
...
if __name__ == "__main__":
    image_folder = os.path.join(os.getcwd(),'aerenchyma_segmentation','data_for_segmentation', 'images', 'val_real')
    ...
    for image_path in image_paths:
        ...
        fig = edge_detector.auto_mask_generate()

        # save the figure with new name
        new_file_name = 'masked_' + file_name
        fig.savefig(os.path.join(image_folder, new_file_name))